...
def AlexNet(train_x, train_y, test_x, test_y):
    ...
    images = tf.placeholder(dtype=tf.float32, shape=[None, 3072])
    labels = tf.placeholder(dtype=tf.float32, shape=[None, 10])

    images_x = tf.reshape(images, [-1, 32, 32, 3])
    #Convelution Layer 1
    with tf.name_scope('conv1') as scope:
        kernel = Kernel([3, 3, 3, 96])
        bias = tf.Variable(tf.constant(0.0, shape=[96], dtype=tf.float32), trainable=True, name='biases')
        conv1 = tf.nn.conv2d(images_x, kernel, strides=[1, 1, 1, 1], padding='SAME', name=scope)
...
def main():
    ...
    X_train, Y_train = Cifar10_Load('./cifar-10/data_batch_1')
    X_test, Y_test = Cifar10_Load('./cifar-10/test_batch')
